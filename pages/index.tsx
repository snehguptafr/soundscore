...
const Home: NextPage = () => {
  ...
  return (
    ...
    <div className='w-9/12 mx-auto my-auto'>
      ...
      <header className="grid grid-cols-3 bg-[#A6BB8D] p-8 radius border-2 border-black rounded-[20px] mt-8">
      ...
      <main className='grid grid-cols-3 gap-4 bg-[#A6BB8D] p-8 radius border-2 border-black rounded-[20px] mt-2'>
        ...
        <div className="p-0 m-0 col-span-1 text-center">
          <span className='font-mono font-bold text-xl'>Recently Played</span>
          {recentSongs && <Spotify link={recentSongs[0]?.track.external_urls.spotify} />}
        </div>

        <div className='col-span-1 text-center'>
          <span className='font-mono font-bold text-xl'>Top Tracks</span>
        ...
        <div className='col-span-1 m-0 p-0 text-center'>
        <span className='font-mono font-bold text-xl'>Top Artists</span>  
          <Carousel showArrows={true} dynamicHeight={false} autoPlay showThumbs={false} showIndicators={false}>
            
            <div className='p-5 border-radius-[20]' key="slide1">
              <img className='p-5' src={topArtists && topArtists[0]?.images[0]?.url} />
              <span className='hover:underline'>{<a href={topArtists && topArtists[0]?.external_urls?.spotify}>{topArtists && topArtists[0]?.name}</a>}</span>
            
            </div>
            <div className='p-5 border-radius-[20]' key="slide2">
              <img className='p-5' src={topArtists && topArtists[1]?.images[0]?.url} />
              <span className='hover:underline'>{<a href={topArtists && topArtists[1]?.external_urls?.spotify}>{topArtists && topArtists[1]?.name}</a>}</span>
            
            </div>
            <div className='p-5 border-radius-[20]' key="slide3">
              <img className='p-5' src={topArtists && topArtists[2]?.images[0]?.url} />
              <span className='hover:underline'>{<a href={topArtists && topArtists[2]?.external_urls?.spotify}>{topArtists && topArtists[2]?.name}</a>}</span>
            </div>
          </Carousel>
        </div>
